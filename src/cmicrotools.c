...
char *cm_str(apr_pool_t *mp, const char *s, apr_size_t sz) {
  ...
  if (mp && s && sz) {
    ...
    result = cm_buffer(mp, s, &bf_size);
  }
  return result;
}

char *cm_trim(apr_pool_t *mp, const char *s) {
  char *result = NULL;
  if (mp && s) {
    ...
    result = cm_str(mp, s + start, end - start + 1);
  }
  return result;
}
...
apr_array_header_t* cm_split(apr_pool_t *mp, const char *s, const char *sp)
{
  ...
  if (str) {
    ...
    while (ptr) {
      *ptr = '\0';
      if (strlen(str) <= 0) {
        tmp = (const char*)cm_empty_string_make(mp);
        if (tmp) {
...
char *cm_datetime(apr_pool_t *mp, apr_time_t t, const char *f)
{
  ...
  if (mp && t && f) {
    if (apr_time_exp_lt(&tm, t) == APR_SUCCESS) {
      fm = apr_pstrdup(mp, f);
      if (fm) {
        fm = cm_str_replace(mp, cm_str_replace(mp, fm, "Y", "%Y"), "y", "%y");
        fm = cm_str_replace(mp, cm_str_replace(mp, fm, "m", "%m"), "d", "%d");
        fm = cm_str_replace(mp, cm_str_replace(mp, fm, "H", "%H"), "h", "%I");
        fm = cm_str_replace(mp, cm_str_replace(mp, fm, "s", "%S"), "i", "%M");
      }
    }
  }
  if (fm) {
...
{
  return cm_file_open(fd, f, APR_READ, mp);
}

apr_status_t cm_file_open_append(apr_file_t **fd, const char *f, apr_pool_t *mp)
{
  return cm_file_open(fd, f, APR_WRITE | APR_CREATE | APR_APPEND, mp);
}

apr_status_t cm_file_open_truncate(apr_file_t **fd, const char *f,
                                   apr_pool_t *mp)
{
  return cm_file_open(fd, f, APR_WRITE | APR_CREATE | APR_TRUNCATE, mp);
}
  ...
  if (mp && fd && buf) {
    ...
    if (fsize <= 0) {
      *buf = NULL;
      ...
      if (fsize > CM_MAX_READ_BUFFER) {
        fsize = CM_MAX_READ_BUFFER;
      }
...
apr_time_t cm_timestamp(int year, int month, int day, int hour,
...
apr_time_t cm_now()
{
  return cm_timestamp(0, 0, 0, 0, 0, 0);
}
...
{
  char *result = NULL;
  char buf[CM_MAX_READ_BUFFER] = {0};
  apr_size_t l;
  apr_file_t *fd;
  apr_size_t bytes = CM_MAX_READ_BUFFER - 1;
...
cm_json_pair_t* cm_json_pair_init(apr_pool_t *mp) {
  cm_json_pair_t *result = NULL;
  if (mp != NULL) {
    if ((result = (cm_json_pair_t*)apr_palloc(mp, sizeof(cm_json_pair_t))) != NULL) {
      //result->pool = mp;
      result->key = NULL;
      result->val = NULL;
      result->type = CM_JSON_T_ZERO;
    }
  }
  return result;
}
...
cm_json_object_t* cm_json_object_init(apr_pool_t *mp) {
  return (cm_json_object_t*)apr_array_make(mp, 0, sizeof(cm_json_pair_t*));
}

int cm_json_object_add(apr_pool_t *mp, cm_json_object_t *jo, cm_json_type_t tp,
                       const char *k, void *v)
{
  int result = 0;
  cm_json_pair_t *entry;
  if ((mp != NULL) && (jo != NULL) && (tp >= 0)) {
    if ((entry = cm_json_pair_init(mp)) != NULL) {
      entry->key = k;
      entry->val = v;
      entry->type = tp;
      APR_ARRAY_PUSH(jo, cm_json_pair_t*) = entry;
      result = 1;
    }
  }
  return result;
...
cm_json_type_t cm_int_type(apr_int64_t v)
{
  if (v < APR_INT32_MIN) {
    return CM_JSON_T_INT64;
  } else if (v < APR_INT16_MIN) {
    return CM_JSON_T_INT32;
  } else if (v <= APR_INT16_MAX) {
    return CM_JSON_T_INT16;
  } else if (v <= APR_UINT16_MAX) {
    return CM_JSON_T_UINT16;
  } else if (v <= APR_INT32_MAX) {
    return CM_JSON_T_INT32;
  } else if (v <= APR_UINT32_MAX) {
    return CM_JSON_T_UINT32;
  } else if (v < APR_INT64_MAX) {
    return CM_JSON_T_INT64;
  } else {
    return CM_JSON_T_ZERO;
  }
}

cm_json_pair_t* cm_json_array_entry_make(apr_pool_t *mp, int type,
                                         const char *key, json_object *val)
{
  cm_json_pair_t *entry = cm_json_pair_init(mp);
  entry->key = key != NULL ? apr_pstrdup(mp, key) : NULL;
  // Eseguo lo switch dei tipi predefiniti di json-c
  switch (type) {
    case json_type_null: {
      entry->type = CM_JSON_T_NULL;
      entry->val = NULL;
    } break;
    case json_type_boolean:{
      entry->type = CM_JSON_T_BOOLEAN;
      entry->val = (void*)apr_palloc(mp, sizeof(char));
      *((char*)entry->val) = json_object_get_boolean(val);
    } break;
    case json_type_double: {
      entry->type = CM_JSON_T_DOUBLE;
...
apr_array_header_t* cm_json_parse(apr_pool_t *mp, json_object *jobj);

apr_array_header_t* cm_json_parse_array(apr_pool_t *mp, json_object *jarr)
{
  int jarr_l;
  enum json_type type;
  ...
  for (int i = 0; i < jarr_l; i ++) {
    ...
    if (type == json_type_array) {
      entry = cm_json_pair_init(mp);
      entry->type = CM_JSON_T_ARRAY;
      entry->key = NULL;
      entry->val = (void*)cm_json_parse_array(mp, jval);
    } else if (type == json_type_object) {
      //entry = (cm_json_pair_t*)apr_palloc(mp, sizeof(cm_json_pair_t));
      entry = cm_json_pair_init(mp);
      entry->type = CM_JSON_T_OBJECT;
      entry->key = NULL;
      entry->val = (void*)cm_json_parse(mp, jval);
    } else {
      entry = cm_json_array_entry_make(mp, type, NULL, jval);
    }
    if (res == NULL) res = apr_array_make(mp, 0, sizeof(cm_json_pair_t*));
    APR_ARRAY_PUSH(res, cm_json_pair_t*) = entry;
  }
  return res;
}
    ...
    switch (type) {
      case json_type_object: {
        if (json_object_object_get_ex(jobj, key, &jtmp)) {
          ...
          entry->val = (void*)cm_json_parse(mp, jtmp);
        }
      } break;
      case json_type_array: {
        if (json_object_object_get_ex(jobj, key, &jtmp)) {
          ...
          entry->type = CM_JSON_T_ARRAY;
          entry->key = apr_pstrdup(mp, key);
          entry->val = (void*)cm_json_parse_array(mp, jtmp);
        }
      } break;
      default: {
        entry = cm_json_array_entry_make(mp, type, key, val);
      } break;
    }
    if (res == NULL) res = apr_array_make(mp, 0, sizeof(cm_json_pair_t*));
    APR_ARRAY_PUSH(res, cm_json_pair_t*) = entry;
...
apr_array_header_t* cm_json_decode(apr_pool_t *mp, const char *s)
{
  json_object *jobj;
  apr_array_header_t* result;
  jobj = json_tokener_parse(s);
  result = cm_json_parse(mp, jobj);
  json_object_put(jobj);
  return result;
}

const char *cm_json_encode(apr_pool_t *mp, const void *v, cm_json_type_t tp)
{
  int len;
  apr_table_entry_t *e;
  apr_table_t *t;
  ...
  if (mp != NULL && tp) {
    if (v == NULL || tp == CM_JSON_T_NULL) {
      ...
      if (obj->nelts > 0) {
        ...
        if (arr != NULL) {
          ...
          for (int i = 0; i < obj->nelts; i ++) {
            switch (type) {
              case CM_JSON_T_NULL: {
                ...
                APR_ARRAY_PUSH(arr, const char*) = apr_pstrdup(mp, CM_JSON_NULL_S);
              } break;
              case CM_JSON_T_BOOLEAN: {
                // Estraggo il intero
                int entry = APR_ARRAY_IDX(obj, i, int);
                // Aggiungo all'array temporaneo una stringa true o false
                APR_ARRAY_PUSH(arr, const char*) = apr_pstrdup(mp, entry ? CM_JSON_TRUE_S : CM_JSON_FALSE_S);
              } break;
              case CM_JSON_T_INT16: {
              ...
              case CM_JSON_T_TABLE: {
                ...
                APR_ARRAY_PUSH(arr, const char*) =
                  ...
                  cm_json_encode(mp, (const void*)entry, CM_JSON_T_TABLE);
              } break;
            }
          }
          ...
          if (arr->nelts > 0) {
            const char *tmp_s = cm_join(mp, arr, ",");
            if (tmp_s != NULL) {
    ...
    } else if (tp == CM_JSON_T_OBJECT) {
      // Il dato è un oggetto (ovvero un array associativo)
      // Un oggetto è una struttura apr_array_header_t di cm_json_pair_t
      // La struttura cm_json_pair_t contiene informazioni anche sul tipo di dato
      ...
      if (obj->nelts > 0) {
        ...
        if ((arr = apr_array_make(mp, 1, sizeof(const char*))) != NULL) {
          ...
          for (int i = 0; i < obj->nelts; i++) {
            ...
            if ((p = APR_ARRAY_IDX(obj, i, cm_json_pair_t*)) != NULL) {
              if (!p->key) continue;
              switch (p->type) {
                case CM_JSON_T_NULL: {
                  // Aggiungo all'array temporaneo una coppia chiave/valore null
                  APR_ARRAY_PUSH(arr, const char*) = apr_psprintf(mp, "\"%s\":%s", p->key, CM_JSON_NULL_S);
                } break;
                case CM_JSON_T_BOOLEAN: {
                  // Aggiungo all'array temporaneo una coppia chiave/valore boolean
                  APR_ARRAY_PUSH(arr, const char*) = apr_psprintf(mp, "\"%s\":%s", p->key, *(char*)p->val ? CM_JSON_TRUE_S : CM_JSON_FALSE_S);
                } break;
                case CM_JSON_T_INT16: {
                ...
                case CM_JSON_T_OBJECT: {
                  // Aggiungo all'array temporaneo una coppia chiave/valore object
                  APR_ARRAY_PUSH(arr, const char*) = apr_psprintf(mp, "\"%s\":%s", p->key, cm_json_encode(mp, p->val, CM_JSON_T_OBJECT));
                } break;
                default: break;
              }
            }
...
cm_logger_t* cm_log_alloc(apr_pool_t *mp, apr_proc_mutex_t *m, const char *f,
                          apr_size_t sz)
{
  cm_logger_t *result = (cm_logger_t*)apr_palloc(mp, sizeof(cm_logger_t));
  if (result != NULL) {
    result->pool = mp;
    result->fname = f;
    result->mutex = m;
    result->max_size = sz ? sz : CM_LOG_MAX_FILE_SIZE;
    apr_status_t st = cm_file_open_append(&(result->fh), f, mp);
    if (st != APR_SUCCESS) {
      return NULL;
    }
    cm_log_rotate(result);
  }
  return result;
}
...
void cm_log_rotate(cm_logger_t *l)
{
  ...
  apr_file_t *fh_new;
  rv = cm_file_open_truncate(&fh_new, l->fname, l->pool);
  if (rv != APR_SUCCESS) {
...
{
  cm_dbd_t *result = NULL;
  if (mp != NULL) {
    if ((result = (cm_dbd_t*)apr_palloc(mp, sizeof(cm_dbd_t))) != NULL) {
      result->drv = NULL;
      result->hdl = NULL;
      result->er_msg = NULL;
      result->trx = NULL;
...
int cm_dbd_open(apr_pool_t *mp, cm_dbd_t *d, const char *s, const char *c)
{
  int result = 0;
  apr_status_t rv;
  d->er_msg = NULL;
...
const char *cm_dbd_escape(apr_pool_t *mp, cm_dbd_t *d, const char *s)
{
...
int cm_dbd_query(apr_pool_t *mp, cm_dbd_t *d, const char *sql)
{
  int result = 0;
...
apr_array_header_t* cm_dbd_result_to_array(apr_pool_t *mp, cm_dbd_t *dbd,
      ...
      while (rv != -1) {
        ...
        for (int i = 0; i < num_fields; i++) {
          ...
          apr_table_set(rec, apr_pstrdup(mp, k),
                        apr_pstrdup(mp, cm_is_empty(v) ? "NULL" : v));
        }
...
int cm_dbd_prepared_query(apr_pool_t *mp, cm_dbd_t *dbd, const char *sql,
                          apr_table_t *args)
{
...
// int cm_dbd_prepared_query(apr_pool_t *mp, cm_dbd_t *dbd,
...
apr_array_header_t* cm_dbd_prepared_select(apr_pool_t *mp, cm_dbd_t *dbd,
  ...
  if ((mp != NULL) && (dbd != NULL) && (sql != NULL) && (args != NULL)) {
    if ((nelts = apr_table_elts(args)->nelts) > 0) {
      if ((args_ar = (char**)apr_palloc(mp, sizeof(char*)*nelts)) != NULL) {
        for (int i = 0; i < nelts; i++) {
          if ((arg = cm_table_entry(args, i)) != NULL) {
        ...
        result = cm_dbd_result_to_array(mp, dbd, res);
      }
    }
  }
  return result;
...
apr_array_header_t* cm_dbd_select(apr_pool_t *mp, cm_dbd_t *d, const char *sql)
{
  int rv, err;
  apr_dbd_results_t *res = NULL;
  apr_dbd_row_t *row = NULL;
...
int cm_dbd_num_records(apr_array_header_t *r) {
  return (int)(r != NULL ? r->nelts : 0);
}

int cm_dbd_num_columns(apr_array_header_t *r) {
  int result = 0;
  apr_table_t *rec;
  if (r && r->nelts) {
...
apr_array_header_t* cm_dbd_column_names(apr_pool_t *mp, apr_array_header_t *r)
{
  int nelts;
  apr_table_entry_t* e;
  apr_table_t *rec;
...
apr_table_t* cm_dbd_record(apr_array_header_t *r, int i)
{
...
const char *cm_dbd_field_value(apr_array_header_t *res, int i, const char *k)
{
...
int cm_dbd_field_set(apr_array_header_t *r, int i, const char *k, const char *v) {
...
const char *cm_dbd_error(cm_dbd_t *d) {
...
{
  cm_http_request_t *result = NULL;
  if (mp) {
    result = (cm_http_request_t*)apr_palloc(mp, sizeof(cm_http_request_t));
    if (result) {
      result->pool = mp;
      result->args = NULL;
      result->body = NULL;
...
apr_table_t *cm_http_request_validate_args(cm_http_request_t *r,
  ...
  if (r && r->args) {
    for (int i = 0; i < nargs; ++i) {
      cm_request_validator_t v = vd[i];
      curr_v = apr_table_get(r->args, v.key);
      if (curr_v == NULL) {
        continue;
      }
      is_valid = 0;
      if (v.type == CM_REQUEST_T_INT) {
        is_valid = cm_is_int(curr_v);
      } else if (v.type == CM_REQUEST_T_DOUBLE) {
        is_valid = cm_is_double(curr_v);
      } else if (v.type == CM_REQUEST_T_STRING) {
        is_valid = !cm_is_empty(curr_v);
      } else if (v.type == CM_REQUEST_T_PASSWORD) {
        is_valid = !cm_is_empty(curr_v);
      } else if (v.type == CM_REQUEST_T_DATE) { // yyyy-mm-dd
        if (!cm_is_empty(curr_v) && strlen(curr_v) == 10) {
          apr_array_header_t *curr_v_ar = cm_split(r->pool, curr_v, "-");
          if (curr_v_ar && curr_v_ar->nelts == 3) {
            ...
            is_valid = strlen(y) == 4 && cm_is_int(y) &&
                       strlen(m) == 2 && cm_is_int(m) &&
                       strlen(d) == 2 && cm_is_int(d);
          }
        }
      }
      if (is_valid) {
        if (v.filter == CM_REQUEST_F_MD5) {
          curr_v = cm_md5(r->pool, curr_v);
        }
...
apr_table_t* cm_http_request_validate_multipart_args(cm_http_request_t *r,
                                                     cm_request_validator_t *vd,
                                                     int nargs) {
  apr_table_t *result = NULL;
  if (r && vd && nargs) {
    ...
    for (int i = 0; i < nargs; ++i) {
      ...
      for (int j = 0; j < r->multipart_data->nelts; ++j) {
        ...
        is_valid = 0;
        if (v.type == CM_REQUEST_T_INT) {
          is_valid = cm_is_int(req_v);
        } else if (v.type == CM_REQUEST_T_DOUBLE) {
          is_valid = cm_is_double(req_v);
        } else if (v.type == CM_REQUEST_T_STRING) {
          is_valid = !cm_is_empty(req_v);
        } else if (v.type == CM_REQUEST_T_PASSWORD) {
          is_valid = !cm_is_empty(req_v);
        } else if (v.type == CM_REQUEST_T_DATE) { // yyyy-mm-dd
          if (!cm_is_empty(req_v) && strlen(req_v) == 10) {
            apr_array_header_t *req_v_ar = cm_split(r->pool, req_v, "-");
...
cm_http_response_t* cm_http_response_alloc(apr_pool_t *mp) {
  cm_http_response_t *result = NULL;
  if (mp) {
    result = (cm_http_response_t*)apr_palloc(mp, sizeof(cm_http_response_t));
  }
  if (result) {
    result->pool = mp;
    result->headers = apr_table_make(mp, 0);
...
const char *cm_http_response_hd_serialize(cm_http_response_t *r) {
  ...
  do {
    if (!r) break;
    int nelts = cm_table_nelts(r->headers);
    if (nelts <= 0) break;
    apr_table_entry_t *e;
    e = cm_table_entry(r->headers, 0);
    result = apr_psprintf(r->pool, "%s: %s\r\n", e->key, e->val);
    for (int i = 1; i < nelts; i++) {
      e = cm_table_entry(r->headers, i);
      if (e) {
...
void cm_http_response_buffer_set(cm_http_response_t *r, void *buf, size_t sz) {
  if (r && buf && sz) {
    r->size = sz;
...
cm_service_t* cm_alloc(apr_pool_t *mp) {
  cm_service_t *result = NULL;
  if (mp) {
    result = (cm_service_t*)apr_palloc(mp, sizeof(cm_service_t));
  }
  if (result) {
    result->pool = mp;
    result->authorized = 0;
...
void cm_route(cm_service_t *s, const char *mth, const char *uri, cm_route_t fn) {
  if (s && mth && uri && fn) {
...
void cm_printf(cm_service_t *s, const char *fmt, ...) {
  va_list args;
  va_start(args, fmt);
...
char *cm_jwt_base64_encode(const unsigned char *s, int sz) {
  char *result = NULL;
  if (s && sz) {
    BIO *bio, *b64;
    BUF_MEM *bufferPtr;
...
char *cm_hmac_encode(const char *key, const char *s, apr_size_t sz) {
  ...
  if (key && s && sz) {
    ...
    HMAC(EVP_sha256(), key, strlen(key), (const unsigned char*)s, sz, hmac, &hmac_len);
    result = cm_jwt_base64_encode(hmac, hmac_len);
  }
  return result;
}
...
char *cm_jwt_token_create(apr_pool_t *mp, apr_table_t *claims, const char *key) {
  ...
  if (mp && claims && key) {
    claims_str = cm_json_encode(mp, claims, CM_JSON_T_TABLE);
  }
  if (claims_str) {
    enc_head = cm_jwt_base64_encode(head, 27);
  }
  if (enc_head) {
    enc_claims = cm_jwt_base64_encode((const unsigned char*)claims_str, strlen(claims_str));
  }
  if (enc_claims) {
    enc_hmac = cm_hmac_encode(key, enc_claims, strlen(enc_claims));
  }
  if (enc_hmac) {
...
int cm_jwt_token_validate(apr_pool_t *mp, const char *tok, const char *key) {
  ...
  if (enc_hmac) {
    gen_hmac = (const char*)cm_hmac_encode(key, enc_claims, strlen(enc_claims));
  }
  if (gen_hmac) {
...
cm_server_t* cm_server_alloc(apr_pool_t *mp) {
  cm_server_t *s;
  s = (cm_server_t*)apr_palloc(mp, sizeof(cm_server_t));
  if (s != NULL) {
    s->pool = mp;
    s->host = NULL;
    s->port = NULL;
...
int cm_dbd_pool_alloc(apr_pool_t *mp) {
  dbd_pool = (cm_dbd_pool_t*)apr_palloc(mp, sizeof(cm_dbd_pool_t));
  return dbd_pool != NULL;
}

void cm_dbd_pool_add(apr_pool_t *mp, const char *drv, const char *conn_s) {
  cm_dbd_t *dbd = cm_dbd_alloc(mp);
  if (dbd != NULL) {
    if (cm_dbd_open(mp, dbd, drv, conn_s)) {
      APR_ARRAY_PUSH(dbd_pool->connections, cm_dbd_t*) = dbd;
      dbd_pool->counter ++;
    }
  }
}
...
int cm_dbd_pool_init(apr_pool_t *mp, const char *drv, const char *conn_s) {
  ...
  if (dbd_pool != NULL) {
    ...
    dbd_pool->connections = apr_array_make(
      mp, CM_DBD_POOL_INIT_SIZE, sizeof(cm_dbd_t*));
    if (dbd_pool->connections != NULL) {
      apr_status_t rv;
      rv = apr_proc_mutex_create(
        &(dbd_pool->mutex), "dbd_pool_mutex", APR_LOCK_DEFAULT, mp);
      if (rv == APR_SUCCESS) {
        for (int i = 0; i < CM_DBD_POOL_INIT_SIZE; ++i) {
          cm_dbd_pool_add(mp, drv, conn_s);
        }
        result = dbd_pool->connections->nelts == CM_DBD_POOL_INIT_SIZE;
      }
    }
  }
  return result;
  ...
  if (rv == APR_SUCCESS) {
    if (dbd_pool->connections->nelts > 0) {
      if (dbd_pool->counter >= 0) {
        dbd = APR_ARRAY_IDX(dbd_pool->connections, dbd_pool->counter, cm_dbd_t*);
        dbd_pool->counter --;
      }
    }
...
void cm_dbd_pool_release() {
  if (dbd_pool != NULL && dbd_pool->connections != NULL) {
    ...
    if (rv == APR_SUCCESS) {
      if (dbd_pool->connections->nelts > 0) {
        if (dbd_pool->counter < CM_DBD_POOL_INIT_SIZE - 1) {
          dbd_pool->counter ++;
        }
      }
      apr_proc_mutex_unlock(dbd_pool->mutex);
  ...
  if (dbd_pool != NULL) {
    ...
    if (rv == APR_SUCCESS) {
      if (dbd_pool->connections != NULL && dbd_pool->connections->nelts > 0) {
        for (int i = 0; i < CM_DBD_POOL_INIT_SIZE; ++i) {
          cm_dbd_t *dbd = APR_ARRAY_IDX(dbd_pool->connections, i, cm_dbd_t*);
...
apr_table_t* cm_http_request_args_parse(apr_pool_t *mp, const char *s, const char *sp1, const char *sp2) {
  apr_table_t *result = NULL;
  if (mp && s && sp1 && sp2) {
    apr_array_header_t *ar = cm_split(mp, s, sp1);
    if (ar && ar->nelts > 0) {
      for (int i = 0; i < ar->nelts; i++ ) {
        const char *entry = APR_ARRAY_IDX(ar, i, const char*);
        apr_array_header_t *pair = cm_split(mp, entry, sp2);
...
apr_table_t* cm_http_request_cookies_parse(apr_pool_t *mp, struct mg_http_message *hm) {
  ...
  if (cookies != NULL) {
    result = cm_http_request_args_parse(mp, cookies->buf, ";", "=");
  }
  return result;
}
...
apr_table_t* cm_http_query_string_parse(apr_pool_t*mp, struct mg_http_message *hm) {
  apr_table_t *result = NULL;
  if (hm->query.len > 0) {
    const char *query = cm_str(mp, hm->query.buf, hm->query.len);
    result = cm_http_request_args_parse(mp, query, "&", "=");
  }
  return result;
}
...
apr_table_t* cm_http_request_body_parse(apr_pool_t*mp, struct mg_http_message *hm) {
  apr_table_t *result = NULL;
  if (hm->body.len > 0) {
    const char *body = cm_str(mp, hm->body.buf, hm->body.len);
    result = cm_http_request_args_parse(mp, body, "&", "=");
  }
  return result;
}
...
int cm_http_request_multipart_parse(apr_pool_t *mp, cm_http_request_t *rq, struct mg_connection *c, struct mg_http_message *hm) {
  ...
  while ((ofs = mg_http_next_multipart(hm->body, ofs, &part)) != 0) {
    ...
    if ((int)part.filename.len > 0) {
      ...
      rv = cm_file_open_truncate(&fd, fpath, mp);
      if (rv == APR_SUCCESS) {
        fsize = cm_file_write(fd, part.body.buf, fsize);
...
void cm_http_request_handler(struct mg_connection *c, int ev, void *ev_data) {
  ...
  do {
    ...
    if (ev == MG_EV_HTTP_MSG) {
      ...
      #endif
      // Server data
      st.server = (cm_server_t*)c->fn_data;
      if (DEBUG) {
        cm_log((st.server)->logger, "INFO", "Client connected");
      }
      {
        apr_status_t rv;
        // APR initialization
        ...
        if (DEBUG) {
          cm_log((st.server)->logger, "INFO", "Service APR initialized");
        }
        ...
        if (DEBUG) {
          cm_log((st.server)->logger, "INFO", "Service pool created");
        }
      }
      // Service allocation
      cm_service_t* sv = cm_alloc(st.pool);
      st.flag.service = sv != NULL;
      if ((st.error = !st.flag.service)) break;
      if (DEBUG) {
        cm_log((st.server)->logger, "INFO", "Service data struct allocated");
      }
      // Logger
      ...
      if (DEBUG) {
        cm_log((st.server)->logger, "INFO", "Service logger initialized");
      }
      // Request
      sv->request = cm_http_request_alloc(st.pool);
      st.flag.request = sv->request != NULL;
      if ((st.error = !st.flag.request)) break;
      if (DEBUG) {
        cm_log((st.server)->logger, "INFO", "Service HTTP request allocated");
      }
      // Request headers
      cm_http_request_headers_set(st.pool, sv->request, st.hm);
      if (DEBUG) {
        cm_log((st.server)->logger, "INFO", "HTTP request headers parsed");
      }
      // Request method
      st.flag.method = (st.hm)->method.len > 0;
      if ((st.error = !st.flag.method)) break;
      sv->request->method = cm_str(
        st.pool, (st.hm)->method.buf, (st.hm)->method.len);
      if (DEBUG) {
        cm_log((st.server)->logger, "INFO", "HTTP request method parsed");
      }
      // Request URI
      st.flag.uri = (st.hm)->uri.len > 0;
      if ((st.error = !st.flag.uri)) break;
      sv->request->uri = cm_str(st.pool, (st.hm)->uri.buf, (st.hm)->uri.len);
      if (DEBUG) {
        cm_log((st.server)->logger, "INFO", "HTTP request uri parsed");
      }
      // Request query string
      if (strcmp(sv->request->method, "GET") == 0) {
        if ((st.hm)->query.len) {
          sv->request->query = cm_str(
            st.pool, (st.hm)->query.buf, (st.hm)->query.len);
          st.flag.query = sv->request->query != NULL;
          if ((st.error = !st.flag.query)) break;
          sv->request->args = cm_http_query_string_parse(sv->pool, st.hm);
          if (sv->request->args) {
            if (DEBUG) {
              cm_log((st.server)->logger, "INFO", "HTTP query string parsed");
            }
          }
        } else {
          st.flag.query = 1;
      ...
      // Request body
      if ((st.hm)->body.len) {
        sv->request->body = cm_str(st.pool, (st.hm)->body.buf, (st.hm)->body.len);
        st.flag.body = sv->request->body != NULL;
        if ((st.error = !st.flag.body)) break;
        sv->request->args = cm_http_request_body_parse(sv->pool, st.hm);
        if (sv->request->args) {
          if (DEBUG) {
            cm_log((st.server)->logger, "INFO", "HTTP body parsed");
          }
        }
      } else {
        st.flag.body = 1;
      }
      {
        ...
        if (cm_in_string(ctype, "multipart/form-data")) {
          st.flag.multipart =
            cm_http_request_multipart_parse(st.pool, sv->request, c, st.hm);
          if ((st.error = !st.flag.multipart)) break;
          if (DEBUG) {
            cm_log((st.server)->logger, "INFO", "HTTP request multipart parsed");
          }
        } else {
          st.flag.multipart = 1;
        }
      ...
      // Response
      sv->response = cm_http_response_alloc(st.pool);
      st.flag.response = sv->response != NULL;
      if ((st.error = !st.flag.response)) break;
      if (DEBUG) {
        cm_log((st.server)->logger, "INFO", "HTTP response allocated");
      }
      // Default response HTTP header Content-Type
      cm_http_response_hd_set(sv->response, "Content-Type", "text/plain");
      if (DEBUG) {
        cm_log((st.server)->logger, "INFO", "HTTP response Content-Type defined");
      }
      // DBD connection
      if ((st.server)->dbd_driver && (st.server)->dbd_conn_s) {
        if (DEBUG) {
          cm_log((st.server)->logger, "INFO", "DBD connection configured");
        }
        sv->dbd = cm_dbd_pool_get();
        st.flag.dbd = sv->dbd != NULL;
        if ((st.error = !st.flag.dbd)) break;
        if (DEBUG) {
          cm_log((st.server)->logger, "INFO", "DBD connection opened");
        }
        st.flag.dbd_handler = sv->dbd->drv != NULL && sv->dbd->hdl != NULL;
        if ((st.error = !st.flag.dbd_handler)) break;
        if (DEBUG) {
          cm_log((st.server)->logger, "INFO", "DBD handler initialized.");
        }
      }
      // ---------------
      // Service handler
      ...
      if (DEBUG) {
        cm_log((st.server)->logger, "INFO", "Service handler executed.");
      }
      cm_dbd_pool_release();
      if (DEBUG) {
        cm_log((st.server)->logger, "INFO", "DBD connection released.");
      }
      cm_http_response_hd_set(sv->response, "Connection", "close");
      // Response headers
      const char *http_hd;
      http_hd = cm_http_response_hd_serialize(sv->response);
      ...
      if (sv->response->status != 200) {
        // Not 200 OK response
        const char *ctype;
        ctype = cm_http_response_hd_get(sv->response, "Content-Type");
        if (ctype) {
...
int cm_server_init(apr_pool_t *mp, cm_server_t **s, int argc, char *argv[], char **er_msg) {
  ...
  do {
    *er_msg = NULL;
    ...
    if ((st.error = !st.flag.input)) break;
    st.flag.args = cm_cmd_args_parse(*s, argc, argv, er_msg);
    ...
    (*s)->logger = cm_log_alloc(mp, st.log_mutex, (*s)->log_file, 0);
...
#define CM_PDF_PADDING_W 6
#define CM_PDF_PADDING_H 12

static void cm_pdf_add_row(double *pos, double x, double y, double h) {
  pos[0] = x ? pos[0] + x : 10.0;
  pos[1] = y ? (pos[3] ? pos[1] + pos[3] + CM_PDF_PADDING_H : pos[1] + y) : 30.0;
  pos[2] = h ? h : 30.0;
  if (pos[3]) pos[3] = 0;
}
...
static void cm_pdf_set_col_border(cairo_t *cr, double *pos, double w, double size, const char *color) {
  cairo_set_line_width(cr, size);
  cairo_set_line_join(cr, CAIRO_LINE_JOIN_MITER);
  cairo_set_source_rgb(cr, cm_ctoi(color[0]), cm_ctoi(color[1]), cm_ctoi(color[2]));
  cairo_rectangle(cr, pos[0], pos[1], w, pos[2]);
  cairo_stroke(cr);
}

static void cm_pdf_set_col_borders(cairo_t *cr, const char *border_pos, double *pos, double w) {
  if (border_pos[0] == '1') {
...
static void cm_pdf_col_fill_text(apr_pool_t*mp, cairo_t *cr, double *pos,
  ...
  if (wrap) {
    ...
    text_ar = cm_split(mp, text, "\r");
  } else {
    text_ar = cm_split(mp, data, "\r");
  }
  if (text_ar->nelts <= 1) {
    cairo_move_to(cr, pos[0] + CM_PDF_PADDING_W, pos[1] + CM_PDF_PADDING_H);
    cairo_show_text(cr, data);
  } else {
    y = pos[1] + CM_PDF_PADDING_H;
    int pos_3 = 0;
    for (int i = 0; i < text_ar->nelts; i++) {
      cairo_move_to(cr, pos[0] + CM_PDF_PADDING_W, y);
    ...
    if (pos_3) {
      ...
      pos[3] = pos_3;
      bord_y[count-1] = pos[3] + CM_PDF_PADDING_H;
    }
  }
}

static void cm_pdf_col_fill_image(cairo_t *cr, double *pos, const char *data, double w, double h) {
  cairo_surface_t *image, *scaled;
  image = cairo_image_surface_create_from_png(data);
  scaled = cm_pdf_scale_image(image, w, h);
...
static void cm_pdf_add_col(apr_pool_t *mp, cairo_t *cr, double *pos, apr_array_header_t *meta, const char *data, double *bord_y, int count) {
  ...
  if (contains_text) {
    cm_pdf_col_fill_text(mp, cr, pos, font_cl, font_name,
                         atof(font_sz), atoi(text_wrap), data, bord_y, count);
  } else if (contains_image) {
    cm_pdf_col_fill_image(cr, pos, data, (double)atof(img_w), (double)atof(img_h));
  }
  pos[0] = pos[0] + atof(col_w);
}

static void cm_pdf_add_col_border(apr_pool_t *mp, cairo_t *cr, double *pos, apr_array_header_t *meta) {
  ...
  if (!have_all_borders) {
    cm_pdf_set_col_borders(cr, border_pos, pos, atof(col_w));
  } else {
    cm_pdf_set_col_border(cr, pos, atof(col_w), atof(border_sz), border_cl);
  }
  pos[0] = pos[0] + atof(col_w);
}

/** @example  const char *buffer;
              cm_file_read(mp, "pdf_file.txt", &buffer);
              cm_pdf_create(mp, buffer, "pdf_file.pdf", 504, 648);
...
static void cm_pdf_add_page(apr_pool_t *mp, cairo_t *dst_cr, apr_array_header_t *page_ar, double h) {
  ...
  if ((page_ar != NULL) && (page_ar->nelts > 0)) {
    ...
    for (int i = 0; i < page_ar->nelts; i++) {
      ...
      if (page_entry != NULL) {
        ...
        page_entry_ar = cm_split(mp, page_entry, "|");
        if (page_entry_ar != NULL) {
          ...
          if (meta_type[0] == 'r') {
            ...
            cm_pdf_add_row(pos, 0, prev_row_h, row_h);
            ...
            cm_pdf_add_col(mp, dst_cr, pos, meta_ar, data, &bord_y[0], count_bord_y);
          }
        }
      }
    }
    ...
    for (int i = 0; i < page_ar->nelts; i++) {
      ...
      if (meta_type[0] == 'r') {
        ...
        cm_pdf_add_col_border(mp, dst_cr, pos, meta_ar);
      }
    }
...
void cm_pdf(apr_pool_t *mp, const char *src, const char *dst, double w, double h) {
  apr_array_header_t *src_ar, *page_ar;
  cairo_surface_t *cr_surf;
  cairo_t *cr;
  // Splitto il tracciato per riga
  src_ar = cm_split(mp, src, "\n");
...
//   sz = cm_file_read(mp, "pdf_file.txt", &buffer, 0, &er);

//   if (sz > 0) {
//     cm_pdf(mp, buffer, "pdf_file.pdf", 504, 648);
//   } else {
...
// module AP_MODULE_DECLARE_DATA cm_qrcode_module;

// const char* cm_qr_map(apr_pool_t *m, QRcode *qr) {
//   int width, len;
//   char *bin, *p;
...
// static int cm_qrcode_request_handler(request_rec *r) {
//   QRcode *qr;
//   const char *buf, *bin, q[] = "HELLO";
//   if (strcmp(r->handler, "qr")) return DECLINED;
//   CM_APACHE_INITIALIZE(r);
//   CM_APACHE_AUTHORIZE(r, &z_qrcode_module);
//   qr = QRcode_encodeString(q, 0, QR_ECLEVEL_L, QR_MODE_8, 1);
//   bin = cm_qr_map(r->pool, qr);
//   buf = apr_psprintf(r->pool, "{\"code\":\"%s\",\"width\":%d}", bin, qr->width);
//   CM_APACHE_RESPONSE_JSON(r, 0, NULL, buf, CM_T_JSON);
//   return OK;
// }

// static void cm_qrcode_register_hooks(apr_pool_t *mp) {
//   ap_hook_handler(z_qrcode_request_handler, NULL, NULL, APR_HOOK_LAST);
// }

// static void* cm_qrcode_serv_config_make(apr_pool_t *m, server_rec *s) {
//   return (void*)apr_table_make(m, 1);
// }

// static const char* cm_qrcode_param_set(cmd_parms *p, void *c, const char *v) {
//   void *cfg = ap_get_module_config(p->server->module_config, &z_qrcode_module);
//   apr_table_setn((apr_table_t*)cfg, p->cmd->name, v);
//   return NULL;
// }

// static const command_rec cm_qrcode_directives[] = {
//   AP_INIT_TAKE1("ZAuthType", cm_qrcode_param_set, NULL, OR_OPTIONS, ""),
//   AP_INIT_TAKE1("ZAuthFile", cm_qrcode_param_set, NULL, OR_OPTIONS, ""),
//   {NULL}
// };
...
//   NULL,
//   NULL,
//   cm_qrcode_serv_config_make,
//   NULL,
//   cm_qrcode_directives,
//   cm_qrcode_register_hooks
// };
